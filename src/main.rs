...
fn analyze_hand(hand: &mut TileSet) {
    ...
    let n_samples = 1 << 21;
    for i in 0..2 {
        let (_, mut discards) = search::discard_tile_by_playout_parallel(hand, &mut wall, i, n_samples);
        discards.sort_by(|(_, s0), (_, s1)| s0.partial_cmp(s1).unwrap());
        println!(
    ...
    let mut discards = uct::evaluate(hand, &wall, n_samples, &mut rand::thread_rng());
